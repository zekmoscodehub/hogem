...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!arePasswordsMatching($password, $confirmPassword)) {
        ...
        $check_username_query = "SELECT * FROM staff WHERE username = '$username' || branch_name = '$branch_name' && user_type = '$user_type'";
        $result = mysqli_query($conn, $check_username_query);

        if (mysqli_num_rows($result) > 0) {
            // Username already exists, set an error message
            $usernameError = "Branch Pastor,Assistant or Username already exists. Please check and try again.";
        } else {
            // Insert the new user into the database
            $insert_query = "INSERT INTO staff (username,email, password, user_type, branch_region, branch_name,title, first_name, last_name, date_assigned, sex, dob, mobile,address, date_joined) VALUES ('$username','$email', '$password', '$user_type','$branch_region', '$branch_name','$title', '$first_name', '$last_name', '$date_assigned', '$sex', '$dob', '$mobile','$address', NOW())";
...
<html lang="en">
    <head>
        ...
        <title>Pastors Registration</title>
...
?>
                                ...
                                <h1 style="padding-left:50px; text-align:center;color:blue; font-family:sans-serif;"><img class="logo"src="assets/img/logo1.png" alt="logo"/>House OF Grace Registration</h1>
...
<div class="form-group">
    ...
    <select class="form-control" name="branch_name" id="branch_name" required>
        <option value="" disabled selected>Select Branch</option>
                                    ...
                                    <div class="form-group">
                                        <label for="date_assigned">Date <span class="login-danger">*</span>(date assigned)</label>
